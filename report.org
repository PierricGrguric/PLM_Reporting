...
*** Work Planned
****** TODO rewrite the dockerfile (split in one with play framework, the other extends from the first with plm)
****** TODO create a depository for the docker image on docker hub
